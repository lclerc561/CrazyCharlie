...
/**
 * Modèle GloutonDescente — Hybride Glouton + Descente de Gradient Stochastique.
 * Phase 1 : Construction gloutonne interactive (meilleur gain marginal).
 * Phase 2 : Optimisation par descente stochastique (Metropolis).
 */
public class ModeleGloutonDescente implements Algo {
    ...
    private boolean effectuerMutation(Composition comp, List<Abonne> abonnes,
        ...
        switch (typeMouvement) {
            ...
            case 2: { // Transfert Box → Box (Compatible)
                ...
                for (Box box : boxes) {
                    if (box != source
                            && box.getAbonne().isArticleCompatible(article)
                            && box.getPoidsTotal() + article.getPoids() <= poidsMax) {
